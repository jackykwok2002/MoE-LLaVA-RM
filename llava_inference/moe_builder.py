...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto",
                          device="cuda", padding_side="left", merge=False, **kwargs):
    ...
    if 'llava' in model_name.lower():
        ...
        if 'lora' in model_name.lower() and 'moe' not in model_name.lower() and model_base is not None:
            ...
            if 'mpt' in model_name.lower():
                ...
                from moellava.model.language_model.stablelm.tokenization_arcade100k import Arcade100kTokenizer
                tokenizer = Arcade100kTokenizer.from_pretrained(model_path, use_fast=False, padding_side=padding_side) # use the same tokenizers as training
                # print(tokenizer)
                if 'moe' in model_name.lower():
                    assert not load_8bit and not load_4bit  # FIXME
                    model = EvalMoELLaVAStablelmForCausalLM.from_pretrained(model_path, use_cache= True, low_cpu_mem_usage=True, attn_implementation="flash_attention_2", **kwargs)
                    import deepspeed
